...
contract EcommerceStore {
  ...
  struct Bid {
    address bidder;
    uint productId;
    uint value; // It is the amount sent by the bidder not the amount being bid
    bool revealed;
  }

  // this following function takes 2 arguments, product Id and encrypyted bid String
  // when following function is invoked, a transaction is created
  function bid(uint _productId, bytes32 _bid) payable public returns (bool) {
    Product storage product = stores[productIdInStore[_productId]][_productId];
    //Check out current block's time stamp
